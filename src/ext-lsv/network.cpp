...
namespace abc_plus {
    ...
    NtkPtr NtkReadBlif(const std::string &i_file) {
        NtkPtr tmp, ntk = Io_ReadBlif((char *) i_file.c_str(), 1);
    ...
    NtkPtr NtkReadBench(const std::string &i_file) {
        NtkPtr tmp, ntk = Io_ReadBench((char *) i_file.c_str(), 1);
        ntk = Abc_NtkToLogic(tmp = ntk);
        Abc_NtkDelete(tmp);
        return ntk;
    }
    ...
    void NtkWriteBlif(NtkPtr ntk, const std::string &o_file) {
        Io_WriteBlifLogic(abc::PLUS_Abc_NtkDup(ntk), (char *) o_file.c_str(), 0); // CHECK
    }
    ...
    void NtkDelete(NtkPtr ntk) { Abc_NtkDelete(ntk); }
    ...
    ObjPtr NtkObjbyID(NtkPtr ntk, int id) { return Abc_NtkObj(ntk, id); }

    ObjPtr NtkPIbyName(NtkPtr ntk, std::string name) { return Abc_NtkFindCi(ntk, (char *) name.c_str()); }

    ObjPtr NtkNodebyName(NtkPtr ntk, std::string name) { return Abc_NtkFindNode(ntk, (char *) name.c_str()); }

    ObjPtr NtkPObyName(NtkPtr ntk, std::string name) { return Abc_NtkFindCo(ntk, (char *) name.c_str()); }
    ...
    bool ObjIsPI(ObjPtr obj) {
        return (bool) Abc_ObjIsPi(obj);
    }

    bool ObjIsPO(ObjPtr obj) {
        return (bool) Abc_ObjIsPo(obj);
    }
    ...
    unsigned int ObjID(ObjPtr obj) {
        return Abc_ObjId(obj);
    }

    NtkPtr ObjHostNtk(ObjPtr obj) {
        return Abc_ObjNtk(obj);
    }

    ObjPtr ObjFanin0(ObjPtr obj) {
        return Abc_ObjFanin0(obj);
    }

    ObjPtr ObjFanin1(ObjPtr obj) {
        return Abc_ObjFanin1(obj);
    }
    ...
    void ObjReplace(ObjPtr obj_old, ObjPtr obj_new) { Abc_ObjTransferFanout(obj_old, obj_new); }
    ...
    ObjPtr ObjCreateInv(ObjPtr fan_in) { return Abc_NtkCreateNodeInv(ObjHostNtk(fan_in), fan_in); }

    void ObjDelete(ObjPtr obj) { Abc_NtkDeleteObj(obj); }
}
